...
void LXeDetectorConstruction::DefineMaterials(){
  ...
  fscint1->AddElement(fC, 47.6*perCent);
  fscint1->AddElement(fH, 52.4*perCent);
  //Fiber(PMMA)
  ...
  G4double scint1_ABSL[]  = { 160.*cm, 160.*cm, 160.*cm};
  ...
  fscint1_mt->AddConstProperty("SCINTILLATIONYIELD",10400./MeV);
  fscint1_mt->AddConstProperty("RESOLUTIONSCALE",1.0);
  fscint1_mt->AddConstProperty("FASTTIMECONSTANT",0.7*ns);
  fscint1_mt->AddConstProperty("SLOWTIMECONSTANT",1.8*ns);
